...
0. **Resolve & load.** Find the PDF; offer to load it (full read into context, or section-by-section). Note ID, folder, basename. Check for an existing annotated copy and prior protocol/scratch entries for this paper.
...
## Standing thesis lenses (refresh from `protocol_vault` + scratchpad — these evolve)

Apply when judging relevance; they were built over prior sessions and live in the vault, so re-read the latest before relying on them.
