...
4. **Recommend 1–3 targets**, plainly: section, why now (one or two sentences each), and what the session would concretely produce (paragraph outline? prose? review pass?). Name the single best next `/thesis-write` invocation.
5. **Also surface** (briefly, at the end): sections that just became blocked or unblocked since last time, and any 🔴→🟡 transitions expected soon (e.g. "S0.7 fleet landing unblocks Ch 5 numbers + Ch 6/7 results").
6. Update the status column in `outline.md` if scoring revealed it is out of date.
