...
4. **House-style** — DBIS norms (hedging modals, unreasoned facts, opinion outside the Conclusion, bonbon scheme) + the LLM-tell list ("not X, it's Y" contrastives, "isn't just X — it's Y", rhetorical em-dash escalations) + terminology drift against `outline.md`'s decisions (attribute vs feature, model names, ids/noid vocabulary). Also the cross-referencing convention (see `/thesis-write`): a claim resting on later-section material without its short forward pointer, backward refs that are narrative recap rather than reuse of a specific definition/notation/result, and re-explaining material that belongs to another section.
