...
3. **Line width — two formats, user picks per call.** The user may say `thin`/`narrow` or `wide` in $ARGUMENTS (e.g. `/annotate wide <topic>`):
   ...
   - **wide** (default if unspecified): hard-wrap at ~80 characters per line — for annotations placed above/below the text or in wide margins. Same total brevity; only the wrapping changes.
