...
### ⚠️ Known issue: image set may be incomplete (2026-06-07)
The raw H&M dataset was consolidated into `data/hm/raw/` from a separate local
folder ("Prototype-Based Explanations for Interpretable Fashion Recommendations"),
and the original 29 GB Kaggle zip was discarded in the process. After the move,
`data/hm/raw/images/` contains only **86 image subdirectories**, whereas
`data/hm/raw/README.md` documents the full set as ~25 GB / ~105,100 images
(subdirectories `000`–`0NN`). The image set is therefore **likely partial**.
Most probable cause: the images were lost earlier, during a Google Drive
download and the re-merging of the split/divided zip parts — not during this
consolidation. The CSVs (`transactions_train.csv`, `articles.csv`, `customers.csv`,
`sample_submission.csv`) appear complete and match their documented sizes.
**If image data is needed, re-download from Kaggle and verify the full
subdirectory range before relying on it.**
