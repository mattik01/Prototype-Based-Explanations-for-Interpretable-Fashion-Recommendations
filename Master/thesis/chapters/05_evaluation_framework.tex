...
\vault{2026-07-15_0943_s07-fleet-complete-dev-tier-baseline-table, 2026-07-15_1359_metric-walkthrough-sharpenings-at-s0-close, 2026-07-14_1338_demo-run-dataset-choice-informed-highlighting-transparent}
% 0943: the S0.7 reference-fleet record (dev tier — the block-3 table's
% working source; thesis-grade numbers re-run per the two-tier policy).
% 1359: D4 history-flatness claim is WINDOW-COMPRESSED (hm_1_month only) —
% scope discipline for every D3/D4 statement; cold-column reading feeds the
% intro's "features inside vs bolted on" gap.
% 1338: if the demo dataset is chosen to highlight an effect, the choice is
% declared transparently (rider recorded).
...
\vault{2026-07-15_1137_score-fidelity-metric-and-eval-section-skeleton, 2026-07-14_1504_fidelity-term-disambiguation-relocated-not-escaped}
% 1137 is the BLUEPRINT for this section's explainability-evaluation part:
% primer (young/unstandardized field, two license quotes) -> what exists
% (coverage-counting vs ground-truth matching) -> what we can't use and why
% -> what we adapt (MEP@theta) and propose (SCORE FIDELITY — Matteo's name,
% graded generalization of Peake & Wang's Model Fidelity; guard sentence
% against the surrogate-agreement misreading).
% DIRECTIVE (1504): wherever the thesis first introduces "fidelity", run the
% disambiguation — mechanism fidelity (attention debate; ours = 1 by
% construction) vs Model Fidelity (P&W) vs Score Fidelity (ours); the fidelity
% problem is RELOCATED to prototype semantics, not escaped.
