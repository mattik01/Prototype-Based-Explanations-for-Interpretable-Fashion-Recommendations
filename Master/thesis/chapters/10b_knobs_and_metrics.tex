...
\vault{2026-07-15_1112_prototype-collapse-is-a-self-built-popularity-bias, 2026-08-15_1639_pm-purity-port-groundtruth-target-discussion, 2026-08-15_1651_spectral-scale-gauge-inversion-direction-diversity-metric, 2026-08-15_1729_r2-latent-signal-port-notes-brief, 2026-08-17_1407_geometry-underdetermined-by-selection-metric-rashomon, 2026-07-15_1137_score-fidelity-metric-and-eval-section-skeleton}
% Geometry-metric rows for the taxonomy (candidates unless stated):
%  - effective direction count / spectral entropy of the row-normalized
%    prototype matrix (1112 committed the blunt count as SC.8 instrument;
%    1651 = the menu + the transferable lesson: an instrument's invariances
%    must match the readout's invariances — cosine readout => row-normalize
%    first, never quote scale-sensitive spectral measures on our prototypes);
%  - neighbourhood purity P_m, ported from F5 (1639, CANDIDATE): measures
%    retention-vs-demotion for us, relative arms only (structural floor from
%    composition); per-field profile as option; ground-truth-column
%    arbitrariness itself a methodological point;
%  - R^2_q latent-signal dominance (1729, CANDIDATE, awareness level):
%    partialed form only (targets correlated);
%  - sim_batch_weight = the worked example of a knob whose effect lives
%    almost entirely on the interpretability axis, invisible to HR@10
%    (geometry-underdetermined entry) — the knob/metric mismatch this
%    chapter catalogs, with our own artifacts;
%  - Score Fidelity (1137): the named fidelity-family row; definition +
%    positioning live in the eval-framework explanation section.
