# dc01 I1 — feature_item_proto: build -> forward -> backward sanity (toy synthetic).
...
# output last-dims equal (user vs item)
with torch.no_grad():
    u_out = rec.user_feature_extractor(u)
    i_out = rec.item_feature_extractor(i)
check("i01.user_item_lastdims_equal", u_out.shape[-1] == i_out.shape[-1] == KU + KT,
...
check("i01.grad_on_user_table",
      user_proto.embedding_ext.embedding_layer.weight.grad is not None
      and user_proto.embedding_ext.embedding_layer.weight.grad.abs().sum().item() > 0, "")
