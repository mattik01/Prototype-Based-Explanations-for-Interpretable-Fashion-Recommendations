...
## SC.10 Cold-read critique (2026-08-20)

> Same-sitting continuation; protocol SC.10 section re-read (GR1). Subagent
> genuinely context-free (GR10): received ONLY the chapter draft +
> `feature_aware_solution_requirements.md`, instructed to read nothing
> else. Brief = the protocol's verbatim four questions, plus the
> learnings-mandated extension (attack overstated equivalences /
> quantifier slippage in gate-born prose) and an explicit
> ratings-vs-evidence audit (too generous AND too harsh both count).

### 1. Critique (verbatim)

**1. [major] The R2 rating converts candor about failure into requirement credit.** §6 grades R2 "strengthened, confirmed," with the basis "the surface truthfully reports popularity where popularity is what the model computes." But R2 (requirements §3) asks for intrinsic *explanations of recommendations* — and the memo's own §5 reports that on the flagship checkpoint "76 fI prototypes carry only 6 distinct intrinsic names," "74/76 per-prototype contributions sit within ±4% of each other," and "Names imply structure that is not there." An explanation surface that honestly reports "12.0% / 11.8%" feature-explained on popular items is an honest *non-explanation* for the head of the catalog. The rating quietly re-interprets R2 from "delivers intrinsic explanations" to "the derivation pipe is intrinsic and truthful about being empty." "Strengthened" — the memo's strongest grade word — is not supported by the memo's own §5 evidence; "partial" is what the evidence shows.

**2. [major] Dual-route concordance is near-vacuous on this checkpoint, yet is cited as confirming evidence.** §5 offers "mean descriptor overlap 0.675, 91% of prototypes share ≥1/3 descriptors" as validation of the intrinsic read-out, and the R2 scorecard row cites "dual-route concordance." But §4.4 reports the activation cloud at effective rank 1.22 of K=76 with "literal duplicates at p99 = 1.0," and §5 says host post-hoc naming is "equally degenerate." When nearly all prototypes are copies pointing at one popularity direction, *any* two sane naming procedures will agree — agreement is a property of the collapse, not of read-out validity. The memo half-concedes this ("validated in the only sense available on this checkpoint") but the scorecard drops the hedge and keeps the credit.

**3. [major] R3 "strong (mechanism)" re-scopes the requirement it grades.** R3 (requirements) demands explanations remain prototype-shaped: "*which prototype(s)*… and *what those prototypes mean*." With 6 names over 76 prototypes and the memo's own reading rule that saturated bars "mark lockstep-clique membership, not single-value meaning" (§5), the "what those prototypes mean" half fails in practice on the only checkpoints examined. Grading the mechanism "strong" and exiling the delivered-meaning failure into a parenthetical "geometry caveat" is a quiet substitution of "prototype-shaped in architecture" for "prototype-shaped in explanation." The evidence supports "partial," not "strong."

**4. [major] The entire cold case rests on a protected evaluation regime, and the limitations section never says so.** §4.3's headline — "Feature composition delivers real, tie-robust cold capability" — and the R5 "strong, confirmed" grade rest exclusively on "cold-vs-cold primary (99-negative pools of cold items)" (§3): cold items ranked only against other cold items. Whether a cold item can ever *win against warm items* — the deployment-relevant question and the natural reading of R5's "meaningful prototype activation" — is unmeasured here. Worse, §4.3 itself leaks that a full-catalog view exists and is unflattering ("noid's cold top-10 remains ~99% signature-twins in the full-catalog view") without presenting any full-catalog numbers. §7 has seven numbered limitations and none mentions the cold-vs-cold protection. This is the memo's largest coverage gap between what results reveal and what limitations own.

**5. [major] R6 is graded against a different bar than the one the requirement states.** R6 (requirements): "match or beat CF-only ProtoMF on the standard (dense) benchmarks… **Beating the paper baseline is the primary thesis bar**." The memo's evidence: parity on hm (not a paper benchmark) and a "real regression" of −0.0555 on ml-1m — the one standard dense benchmark actually run. §2's "a staged bar, not a lowered one" is assertion by fiat: for *this* memo, deferring the UI-ProtoMF paper bar and grading against the I-host means the requirement's own stated primary bar goes unassessed, and on the bar's home dataset the candidate loses to the host, to both LightFM rows, and nearly everything except mf. "Partial — split verdict" launders a per-the-requirement failure into an interesting finding. The regime split is genuinely interesting; the *grade* is generous.

**6. [major] "The two testbeds disagree, and that is the finding" reifies a two-point, one-seed-each contrast into a "regime boundary."** §4.1/§4.2 name a boundary (thin×rich vs heavy×coarse) from exactly one dataset per regime, one seed per dataset, with a confessed confound on one side (popularity-sampled training negatives in fI's best ml-1m trial), and with the memo itself admitting the regimes differ on at least two covarying axes (twin density 67.5% vs 3.8%; median history 5 vs 56). Two datasets that differ in everything cannot locate a boundary; they can only exhibit a difference. The scorecard's "regime boundary named" (R6 row) treats the interpretation as a result.

**7. [major] The §2 headline interpretability claim is stated unconditionally while the memo's own limitations gut its scope.** §2A: "prototype meaning is read from parameters alone… every activation splits *exactly* into named per-row summands… covers **100% of the item-discriminating score**." Exactness of an arithmetic decomposition is not attribution validity: §7.3 concedes per-value shares are "gauge-like" and only clique-level readings are identified; §7.2 concedes names over the collapsed space "imply structure that is not there"; each summand also depends on all other rows through ‖q_i‖. So what §2 sells as the candidate's new axis is, on the memo's own pages, exact arithmetic over labels whose individual meaning is unidentified and whose profiles are degenerate on the flagship checkpoint. The claim needs its conditions attached at the point of claim, not five pages later.

**8. [minor] The printed brackets contradict the memo's own tightness rule.** §3: "All claim-carrying brackets below are tight (width ≲ 0.001)." The §4.3 table's widths: lightfm_tags_ids 0.0015, fI-noid 0.0017, kNN-patch 0.0014, fI 0.0013, lightfm_tags 0.0011. Four of five exceed the stated bound, one by 1.7×. No conclusion flips (brackets stay disjoint), but a memo this rule-proud should not fail its own printed rule in the same document.

**9. [minor] §4.3 violates the spirit of the memo's own §3.6 quoting rule.** The memo forbids quoting the ids−noid gap as "the cost of interpretability" because the gap is undecomposed — then §4.3 quotes the LightFM−fI cold gap as "what the prototype bottleneck costs at cold," an equally undecomposed cross-architecture difference (training, loss, and search all differ) attributed to the bottleneck by narrative fiat. Same sin, different pair.

**10. [minor] fI trails `lightfm_tags_ids` on warm hm by 0.0103 (~4× the memo's own noise yardstick) and the prose never says so.** The §4.1 table prints it honestly, and the memo *does* narrate fI-vs-LightFM where it happens on ml-1m ("fI also trails both lightfm rows there") and at cold. Omitting the same comparison only on the primary testbed's warm table — the memo's best-looking result — is selective narration. GR5 is no shield: LightFM is a baseline, not a candidate.

**11. [minor] The noise yardstick is one draw against a frozen row from a different wave, then used as a ratio denominator.** §3/§4.1: |f0 − item_proto| = 0.0026 is a single realized difference, where f0 got its own fresh 30-trial hyperopt and item_proto is a frozen S0.7 number "never re-run" — the draw conflates hyperopt stochasticity, wave drift, and wrapper/init noise. The memo caveats it correctly ("one draw, a scale indicator, not a CI"), then spends the caveat immediately: "~2×," "~11× the yardstick" and "candidate real effect" are pseudo-precision built on n=1. Separately, "beats mf" on ml-1m (+0.0134) is quoted with no noise qualifier at all — the one comparison that escapes the parity discipline is a favorable one.

**12. [minor] The §4.2 "cross-dataset consistency" argument inherits the confound the memo attached elsewhere.** The 3.9× hm/ml-1m gap ratio is used as evidence that popularity/twin components dominate taste-structure loss — but the ml-1m fI number in that ratio is the same run carrying the popularity-sampled-negatives confound flagged in §4.1, which is not re-attached here; and with two named axes covarying across the same two datasets (twins *and* history depth), "directionally consistent" distinguishes nothing.

**13. [minor] "The collapse is not data-fated" (bolded, §4.4) outruns the evidence in its own sentence.** The support is an admittedly "uncontrolled comparison (different K/d/population)" on a *different dataset* (hm_3_month) — which cannot disprove that collapse is fated on hm_1_month; the isolating experiment is "recorded but not queued." A conditional observation is typeset as an absolute.

**14. [minor] S5 "strong, confirmed" sits on an unreconciled cost discrepancy.** §1 claims "~1.05–1.1× step cost"; §6 cites "fI 2h37 vs f0 1h50 hyperopt" — a 1.43× wall-time ratio. Different trial counts or early stopping could explain it, but the memo says nothing, and prints both numbers.

**15. [minor] R1 "strong, confirmed" is strong only against a requirement amended at this candidate's own gate.** The requirements doc's R1 clarification (2026-07-12, "recorded at the dc01 SC.1b-delta gate") relocates the founding weight-sharing exemplar to the merge stage; against the original text ("mirroring the `prototypes_double_tie` philosophy where the… branches share weights"), a single-branch fI with a free user vector is at best partial at this stage. The amendment is disclosed and dated — this is done in the open — but "a clarification, not a weakening" is, like "staged bar, not a lowered one," the project grading its own goalpost move.

**16. [nit] "Clear the chance floor by 2.4–3.2×" computes against the *popularity reference* (0.1202) while the table's own last row defines chance as 0.10.** The two floors are distinct concepts and the bullet conflates their names; the popularity reference is also the only cold number quoted without a bracket.

**17. [nit] The scorecard's R1 one-liner "f0 ≈ host to 3 digits" is ambiguous and, read as accuracy, false.** The warm table has f0 0.4912 vs host 0.4938 — differing in the third decimal. §4.4 makes clear the three-digit claim is about geometry statistics; the compressed scorecard line loses that scope. Likewise "No self-rating was contradicted by evidence" (§6) is the grader auditing itself and, per points 1, 3, and 5 above, not true under the requirements' own text.

**Closing assessment.** *Is this sound?* Methodologically it is unusually disciplined for a working memo — tiered number policy, tie brackets, pinned keystone reduction, forbidden-quote rules — and the arithmetic as printed checks out almost everywhere (deltas, ratios, and bracket disjointness all recompute correctly; the bracket-width rule in §3 is the one printed self-contradiction). *Are the claims supported?* The measurement claims mostly are; the *interpretive and graded* claims are not uniformly: the cold headline rests on a protected cold-vs-cold regime the limitations never own (point 4), the "regime boundary" is a two-point extrapolation (6), and the interpretability story is graded on mechanism while the delivered explanation surface is degenerate on the only checkpoints examined (1–3, 7). The scorecard's systematic tilt is generous-by-rescoping — R2, R3, R6, and R5 each grade a quietly narrowed reading of the requirement text; I found no too-harsh rating (R4 "partial" and R7 "partial" are fair, arguably even lenient on R4). *What is missing or overclaimed?* Missing: full-catalog cold numbers (or an explicit limitation), acknowledgment of the lightfm_tags_ids warm-hm deficit, reconciliation of the S5 cost figures, and engagement with R6's "paper baseline is the primary thesis bar" sentence. Overclaimed: "strengthened/strong" on R2/R3, "not data-fated," the regime boundary, and the unconditional §2 attribution headline. *Could this enter a master's thesis?* In its intended role — a results-memo feeding a chapter, not thesis prose — **yes, conditionally**: the evidence base, instruments, and candor are genuinely chapter-grade raw material, and the memo correctly bars its own numbers from thesis quotation. But a chapter drafted from it as-is would inherit inflated interpretability grades and an undisclosed cold-evaluation scope restriction; points 1–7 need repair (regrade R2/R3/R6, surface the cold-vs-cold protection in limitations, demote "boundary" to "difference") before this feeds prose.

### 2. Response (concede / rebut / open risk, point by point)

- **P1 — partly concede, partly rebut.** Rebut: R2's own text asks for
  *derivability at inference time* (intrinsic vs post-hoc reconstruction),
  not explanation quality — quality is graded where the memo's caveats
  live (R3 meaning, R4 sharpness, R7 vocabulary); "strengthened" is the
  re-host delta (the Block-I/Block-U opacity split dissolving), a
  mechanism fact. Concede: the grade word invites the quality reading and
  the scorecard dropped the §5 hedge. → Fix F1: R2 basis re-scoped in the
  row itself ("derivability scope; explanation *quality* governed by the
  R3/R4 caveats"), hedge restored.
- **P2 — concede.** Route agreement over a ~1-D collapsed space is weak
  (necessary-not-sufficient) validation; the hedge must travel with every
  citation. → Fix F2.
- **P3 — concede the presentation; propose a split grade.** The "what the
  prototypes mean" half fails in practice on this checkpoint. → Fix F3:
  R3 → "strong (mechanism) / **partial (delivered, this checkpoint)**" —
  the re-scope made explicit in the grade, not a parenthetical.
- **P4 — concede; the strongest point.** And the disclosure can be
  concrete: the cold-vs-all secondary numbers EXIST in the landed reports
  (`cold_eval_brackets/` mirrors) and were simply not quoted. fI 0.2179
  [.2178–.2181], noid **0.3935** [.3930–.3944] (holds ≈ its cold-vs-cold
  0.3882 against warm competition), lightfm_tags 0.4700, tags_ids 0.5082,
  CF native rows 0.0000 (saturated warm scores shut cold items out
  entirely), popularity 0.0000 (true zero: cold pop = 0). → Fix F4:
  (a) §4.3 gains the cold-vs-all secondary rows; (b) new §7 limitation
  naming the cold-vs-cold protection explicitly. Note the disclosure
  *strengthens* noid's story and moderates fI's — exactly why it belongs
  in.
- **P5 — partly rebut, partly concede.** Rebut "laundering": the staged
  bar is a ratified, dated lineage decision (SC.1b-delta P7), disclosed
  in the memo; grading fI against UI-ProtoMF would compare across a
  mechanism this stage deliberately lacks. Concede: the memo must engage
  the requirement's own sentence — the primary paper bar is **unassessed
  at this stage**, not met; and unlike R1, the requirements doc carries
  no dated staging note for R6. → Fix F5: §2 + R6 row say "unassessed by
  the ratified staging decision — deferred, not met"; **optional user
  decision: add the R1-precedent dated staging clarification note to the
  requirements doc's R6** (it is a living document; the note records the
  2026-07-12 decision, it does not invent a new one).
- **P6 — concede.** Two datasets differing on covarying axes exhibit a
  difference; they cannot locate a boundary or identify the driver. →
  Fix F6: "regime boundary" → "regime difference" throughout; R6 row
  reworded.
- **P7 — concede the presentation.** Exactness ≠ attribution validity; the
  conditions must sit at the point of claim. → Fix F7: §2A gains the
  scope sentence (gauge/identifiability, collapse, ‖q_i‖ entanglement —
  pointers to §7.2/7.3).
- **P8 — concede (trivial).** The rule was written as full-width, the
  brackets were measured as ±half-width. → Fix F8: "half-width ≤ ~0.001
  (full width ≤ 0.002)".
- **P9 — concede.** Same sin, different pair, correctly caught. → Fix F9:
  the LightFM−fI cold gap is a cross-architecture difference; the
  bottleneck reading becomes a named candidate hypothesis, not an
  attribution. §4.3 + §7.5.
- **P10 — concede.** → Fix F10: §4.1 narrates fI vs lightfm_tags_ids on
  warm hm (−0.0103, ~4× the yardstick draw).
- **P11 — partly concede.** The caveat exists but is spent immediately;
  and "beats mf" escaped the discipline. → Fix F11: one "ratios
  indicative under n=1" clause; the mf comparison gets the same
  single-seed qualifier.
- **P12 — concede.** → Fix F12: §4.2 re-attaches the popular-negatives
  confound and states the covarying axes make "directionally consistent"
  non-discriminating between the components.
- **P13 — concede the wording.** → Fix F13: "not data-fated" → "not
  demonstrated to be data-fated": the probe refutes
  inevitability-under-any-config in a neighboring regime; whether
  hm_1_month's collapse is avoidable is exactly the recorded (unqueued)
  isolating rerun's question.
- **P14 — partly rebut, concede presentation.** Hyperopt walltime
  confounds per-trial config draws and early stopping — it is not a
  step-cost measurement; but the memo printed both numbers without
  saying so. → Fix F14: reconciliation clause.
- **P15 — rebut the re-grade, concede the citation.** The R1
  clarification is dated, ratified, and disclosed in the requirements doc
  itself — the asymmetry-declared discipline working as designed, not a
  goalpost move to hide; fI is graded against R1's operative core, which
  the requirement's own note names as the per-stage criterion. → Fix F15:
  the R1 row cites the dated note explicitly so a cold reader finds it.
- **P16 — concede (nit).** → Fix F16: the two floors disambiguated
  (popularity reference 0.1202, degenerate bracket, quoted as such;
  analytic chance 0.10); multiples restated against both.
- **P17 — concede (nits).** → Fix F17: R1 one-liner scoped "(geometry
  statistics)"; the §6 "no self-rating contradicted" sentence is replaced
  by an honest delta record: the SC.10 pass re-scoped R2/R3/R6's
  presentation (grades adjusted per F1/F3/F5) — the design doc's own
  ratings were optimistic in *presentation*, not in measurement.

**Open risks (no fix, recorded):** none new — every conceded point is a
presentation/disclosure repair; no measurement was falsified (the critic's
own closing: arithmetic recomputes correctly throughout).

### 3. Proposed verdict (dev-profile-provisional, absolute phrasing)

The candidate **survives the cold read in its intended role**. The
measurement layer is sound: numbers recompute, the discipline apparatus
(tiers, brackets, keystone, forbidden quotes) held. The interpretive and
grading layer required SC.10 correction: the cold-vs-cold scope is now
disclosed with its cold-vs-all numbers, the R2/R3/R6 grades carry their
re-scopes in the grade text itself, and the regime claim is demoted from
boundary to difference. With fixes F1–F17 applied, the memo is
chapter-grade raw material for the fI stage — conditional pass, exactly
as the critic's closing put it, all of it dev-profile-provisional and
none of it thesis-quotable numbers.

### Gate (SC.10) — decisions requested

1. Accept the point-by-point response (concessions/rebuttals as listed).
2. Approve fix bundle F1–F17 applied to the chapter draft (grade-text
   changes: R2 re-scoped, R3 split, R6 "unassessed primary bar" wording).
3. **Optional:** R6 staging clarification note in the requirements doc
   (R1 precedent) — yes/no.
4. Ratify the verdict as recorded.

**Gate CLOSED 2026-08-20 (user: "yes sure" — all four decisions approved).**
Application record: fix bundle F1–F17 applied to the chapter draft same
sitting — §2A scope-at-claim sentence (F7); §2 "unassessed primary bar"
(F5); §3 bracket half-width correction (F8); §4.1 lightfm_tags_ids warm-hm
narration (F10), n=1 ratio clause + mf qualifier (F11), boundary→difference
(F6); §4.2 confound re-attachment + non-discrimination statement (F12);
§4.3 cold-vs-all secondary block + floors disambiguated + popularity
bracket + bottleneck-cost demoted to hypothesis (F4a/F16/F9); §4.4 "not
demonstrated to be data-fated" (F13); §5 concordance hedge (F2); §6 delta
record replacing the "no self-rating contradicted" sentence (F17), R1 row
citation+scope (F15/F17), R2 re-scoped grade (F1), R3 split grade (F3),
R5 secondary-disclosed basis, R6 "primary bar unassessed" (F5), S5
walltime reconciliation (F14); §7 new limitation 9 — cold-vs-cold
protection (F4b). **Decision 3 (optional) taken YES:** dated R6 staging
clarification note added to `feature_aware_solution_requirements.md`
(R1-note precedent; documents the 2026-07-12 decision, alters nothing).
**Verdict ratified as recorded in §3: conditional pass,
dev-profile-provisional.** → Next: **SC.11 final flush**.

## SC.11 Final flush (2026-08-20)

> Same-sitting continuation; protocol SC.11 section re-read (GR1).

### 1. Learnings flush + full compaction

One new entry flushed (`[dc01 SC.9/SC.10]`: vault-sweep before drafting;
testbed parity; secondary-regime disclosure). **Full compaction executed**
(owed since the SC.7 no-compaction call): 31 entries → 21, merges only
(docs-drift trio, eval-degeneracy pair, config-default pair, adversary
pair, host-inheritance trio, empirically-not-scholastically pair,
migration pair, cluster-ops quartet); no substance dropped, origin tags
preserved, pre-compaction version in git history.

### 2. Protocol self-evaluation (timeboxed, 2 proposals — user decisions)

- **P1 — SC.9 input list gains two lines (applies from the next SC.9
  onward, i.e. dc05):** (a) mandatory direct protocol-vault sweep
  (candidate-relevant + pause-period entries) before drafting — the dc01
  SC.9 gate caught 4 missing entries that the dossier-only draft basis
  produced; (b) SC.9.1's content list amended: results presented for
  every in-scope testbed at equal weight, and every committed secondary
  eval regime quoted at the headline or its absence justified — the SC.10
  cold reader's strongest point (P4) was exactly this omission.
- **P2 — SC.10's brief gains the standing extensions that earned their
  keep this pass (applies from dc05 onward):** the
  ratings-vs-evidence audit (too generous AND too harsh) and the
  grades-carry-their-scope attack are written into the SC.10 brief text
  rather than re-derived from the learnings ledger each time.
- No third proposal. What worked without friction: the verbatim-attach +
  point-by-point response format; the context-free subagent (its critique
  was materially different from every prior first-party pass); the fix
  policy (all 17 points resolved in one gate).

### 3. Candidate index

dc01 row → **scrutinized (dev-profile)**, dated 2026-08-20 (applied).

### 4. Closing summary (5 lines)

1. **fI-ProtoMF is scrutinized end-to-end (dev-profile):** concept
   (re-hosted UI→I, F-DC01-01..09), implementation (t01–t12 + keystone),
   explanations (shared renderer + cards, 3 novelties, no backports),
   runs (11-job fleet, charter-clean 11/11), results verified (SC.8),
   memo drafted (SC.9), cold-read passed conditionally (SC.10, repairs
   applied).
2. **Headline evidence:** hm warm parity with the host (+0.0046, ~2×
   yardstick) + possible N@10 gain; ml-1m regression (−0.0555, confound
   disclosed); cold tie-robust above floors and patched host, below the
   LightFM bar (brackets tight; noid holds in cold-vs-all).
3. **Mechanism story measured:** ids-noid gap = head-bucket popularity
   phenomenon with tail refunds + thin-user inversion (D3/D4); activation
   clouds near-collinear on all arms (collapse = self-built popularity
   bias, config-dependent per the coverage-knob probe); twin
   individuation real on the ids arm.
4. **Explanation surface:** honest and exact (renderer self-checks,
   feature-explained fraction carries real signal 12%↔86.5%), but
   collapse dominates both models' surfaces — R3 delivered-half partial
   on this checkpoint; routes to the Rashomon/coverage stage.
5. **Everything dev-provisional, nothing thesis-quotable;** artifacts:
   design doc (amended), this dossier, `sc01_chapter_draft.md`, findings
   ledger F-DC01-01..13 + F-S0-07/14/15/16, learnings (compacted),
   frozen-table comparisons under R-a..R-g + bracket layer.

**Dossier closed.** dc01's scrutiny routine is complete. The user decides
the next move (per protocol: next candidate's post-run block — dc05
resumes at its own SC.7 — or protocol exit). Preservation rule: this
dossier and its ledgers are retained; copies route to `Master/llm_usage/`
at phase close per the CLAUDE.md at-risk-artifact rule.
