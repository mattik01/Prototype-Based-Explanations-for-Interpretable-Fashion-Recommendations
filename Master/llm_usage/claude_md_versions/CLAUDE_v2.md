...
**Models** (`-m`): baselines `mf`, `acf`, `user_proto`, `item_proto`, `user_item_proto`; feature-aware `feature_item_proto` (+ `_noid`/`_f0` ablations), `attr_item_proto`; CBF baseline `lightfm_tags`, `lightfm_tags_ids`
